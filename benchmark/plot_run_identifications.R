...
get.run.identifications = function(report, id_column, run_groups = NULL) {
  ...
  ident_numbers = do.call(rbind, lapply(1:length(run_names), function(j) {
    ident_names = ident_names[run_names[[j]]]
    
    run_counts = table(unlist(ident_names))
    ident_numbers = do.call(rbind, lapply(1:length(ident_names), function(i) {
...
plot.run.identifications = function(report, id_column, ..., 
                                    filename = 'run_identifications.svg',
                                    width = 12, height = 8, unit = 'cm',
                                    return_data = FALSE) {
  run_identifications = get.run.identifications(report, id_column, ...)
